		...
		try {
			...
			if ((packet instanceof ICMPPacket) || (packet instanceof IGMPPacket) || (packet instanceof TCPPacket)
				...
				if (packet instanceof ICMPPacket) {
					...
					icmp++;
					RFcaptura.pkether(
							(new StringBuilder(String.valueOf(strNumero))).append(icmpPacket.toString()).toString());
				}
				if (packet instanceof IGMPPacket) {
					IGMPPacket igmpPacket = (IGMPPacket) packet;
					igmp++;
					RFcaptura.pkIGMP(
							(new StringBuilder(String.valueOf(strNumero))).append(igmpPacket.toString()).toString());
				}
				if (packet instanceof TCPPacket) {
					TCPPacket tcpPacket = (TCPPacket) packet;
					tcp++;
					RFcaptura.pkTCP(
							(new StringBuilder(String.valueOf(strNumero))).append(tcpPacket.toString()).toString());
				}
				if (packet instanceof UDPPacket) {
					UDPPacket udpPacket = (UDPPacket) packet;
					udp++;
					RFcaptura.pkUDP(
							(new StringBuilder(String.valueOf(strNumero))).append(udpPacket.toString()).toString());
				}
				...
				if (packet instanceof ARPPacket) {
					...
					arp++;
					RFcaptura.pkARP(
							(new StringBuilder(String.valueOf(strNumero))).append(arpPacket.toString()).toString());
				}
				if (packet instanceof IPPacket) {
					IPPacket ipPacket = (IPPacket) packet;
					ip++;
					RFcaptura.pkIP(
							(new StringBuilder(String.valueOf(strNumero))).append(ipPacket.toString()).toString());
				}
			} else if (packet instanceof EthernetPacket) {
				EthernetPacket ethernetPacket = (EthernetPacket) packet;
				ethernet++;
				RFcaptura.pkether(
						(new StringBuilder(String.valueOf(strNumero))).append(ethernetPacket.toString()).toString());
			}
