...
public class Rules {
		...
		try {
			...
			for (BufferedReader lector = new BufferedReader(new FileReader(fichero)); lector.ready();) {
				...
				if (cadena.indexOf("#") == 0 || cadena.length() == 0) {
					...
					for (String char_ini = cadena.substring(0, 1); char_ini.compareTo("(") != 0; char_ini = cadena
							.substring(0, 1)) {
	...
	public void DatosRules(String IpOrigen, String IpDestino, String portsrc, String portdest, String protocol,
			byte data[], String itype, String icode, String dsize, String id, String ttl, String flags, String ack,
			String seq, Packet packet, int numerodepaquete) {
		...
		for (int i = 0; i < VectorRules.size() && !rule_existente; i++) {
			...
			if (objetoRule.getDireccion().equals("->")) {
				if ((IpOrigen.equals(objetoRule.getIpSrc()) || objetoRule.getIpSrc().equals("any"))
						&& (IpDestino.equals(objetoRule.getIpDst()) || objetoRule.getIpDst().equals("any"))
						&& (portsrc.equals(objetoRule.getPortSrc()) || objetoRule.getPortSrc().equals("any"))
						&& (portdest.equals(objetoRule.getPortDest()) || objetoRule.getPortDest().equals("any"))) {
					rule_existente = TratarPropiedades(objetoRule, protocol, data, itype, icode, dsize, id, ttl, flags,
							ack, seq, numerodepaquete);
					if (rule_existente) {
						System.out.println(
								(new StringBuilder("ALerta Contenido ===>>>")).append(rule_existente).toString());
						System.out.println((new StringBuilder("ALerta Contenido MSG===>>>")).append(objetoRule.getMsg())
								.append("paquete:").append(numerodepaquete).toString());
						log.addAlert(objetoRule, numerodepaquete, packet, IpOrigen, IpDestino, portsrc, portdest);
						TablaAlertas.DatosTablaAlerts(objetoRule, numerodepaquete, packet, IpOrigen, IpDestino, portsrc,
								portdest);
					}
				}
			} else if (objetoRule.getDireccion().equals("<>") && ((IpOrigen.equals(objetoRule.getIpSrc())
					|| objetoRule.getIpSrc().equals("any"))
					&& (IpDestino.equals(objetoRule.getIpDst()) || objetoRule.getIpDst().equals("any"))
					&& (portsrc.equals(objetoRule.getPortSrc()) || objetoRule.getPortSrc().equals("any"))
					&& (portdest.equals(objetoRule.getPortDest()) || objetoRule.getPortDest().equals("any"))
					|| (IpOrigen.equals(objetoRule.getIpDst()) || objetoRule.getIpDst().equals("any"))
							&& (IpDestino.equals(objetoRule.getIpSrc()) || objetoRule.getIpSrc().equals("any"))
							&& (portsrc.equals(objetoRule.getPortDest()) || objetoRule.getPortDest().equals("any"))
							&& (portdest.equals(objetoRule.getPortSrc()) || objetoRule.getPortSrc().equals("any")))) {
				rule_existente = TratarPropiedades(objetoRule, protocol, data, itype, icode, dsize, id, ttl, flags, ack,
						seq, numerodepaquete);
				if (rule_existente) {
					log.addAlert(objetoRule, numerodepaquete, packet, IpOrigen, IpDestino, portsrc, portdest);
					TablaAlertas.DatosTablaAlerts(objetoRule, numerodepaquete, packet, IpOrigen, IpDestino, portsrc,
	...
	public boolean TratarPropiedades(Rule objetoRule, String protocol, byte data[], String itype, String icode,
			String dsize, String id, String ttl, String flags, String ack, String seq, int numerodepaquete) {
		int j = 0;
		...
		try {
			if (data.length != 0) {
				for (; j < objetoRule.getVectorContent().size() && alert_contiene; j++) {
					...
					if (char_ini.equals("|")) {
						...
						for (int i = 0; i < str1.length; i++)
							Contenido_std = (new StringBuilder(String.valueOf(Contenido_std))).append(str1[i])
									.toString();
						...
						} else if (datospk.indexOf(Contenido_std) != -1)
							alert_contiene = false;
						else
							alert_contiene = true;
					} else if (contiene) {
						...
						if (datospk.indexOf(Contenido) != -1)
							...
							alert_contiene = false;
					} else if (datospk.indexOf(Contenido) != -1)
						alert_contiene = false;
					else
						alert_contiene = true;
				}
				...
				if (objetoRule.getProtocolo() != null && !objetoRule.getProtocolo().equals(protocol))
					alert_contiene = false;
				else if (objetoRule.getitype() != null && !objetoRule.getitype().equals(itype))
					alert_contiene = false;
				else if (objetoRule.geticode() != null && !objetoRule.geticode().equals(icode))
					alert_contiene = false;
				else if (objetoRule.getdsize() != null && !objetoRule.getdsize().equals(dsize))
					alert_contiene = false;
				else if (objetoRule.getid() != null && !objetoRule.getid().equals(id))
					alert_contiene = false;
				else if (objetoRule.getTtl() != null && !objetoRule.getTtl().equals(ttl))
					alert_contiene = false;
				else if (objetoRule.getFlags() != null && flags.indexOf(objetoRule.getFlags()) == -1)
					alert_contiene = false;
				else if (objetoRule.getack() != null && !objetoRule.getack().equals(ack))
					alert_contiene = false;
				else if (objetoRule.getseq() != null && !objetoRule.getseq().equals(seq))
					alert_contiene = false;
			} else {
				alert_contiene = false;
			}
