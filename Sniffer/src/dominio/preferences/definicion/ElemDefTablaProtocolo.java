...
public class ElemDefTablaProtocolo extends Element {
	...
	private String convertirAlfanumerico(String cadena) {
		...
		for (int i = 0; i < j; i++) {
			char letra = ac[i];
			binario = (new StringBuilder(String.valueOf(binario)))
					.append(String.format(Integer.toBinaryString(letra), new Object[0])).toString();
		}
	...
	private String convertirNumerico(String cadena) {
		return String.format("%16s", new Object[] { Integer.toBinaryString(Integer.valueOf(cadena).intValue()) });
	}
