	...
	private boolean checkValoresBoolean() {
		...
		for (int i = 0; i < valores.length && ok; i++)
			if (valores[i][3] == null || String.valueOf(valores[i][3]).equals(""))
				ok = true;
			else if (String.valueOf(valores[i][5]).equals("Booleano")) {

				if (String.valueOf(valores[i][3]).toLowerCase().equalsIgnoreCase("false")
						|| String.valueOf(valores[i][3]).toLowerCase().equalsIgnoreCase("falso")
						|| String.valueOf(valores[i][3]).toLowerCase().equalsIgnoreCase("true")
						|| String.valueOf(valores[i][3]).toLowerCase().equalsIgnoreCase("verdadero")) {
					ok = true;
				} else {
					ok = false;
					med.lanzaraviso(mensaje, "Error");
	...
	private boolean checkNumericos(String valor) {
		if (valor.indexOf("-") > 0) {
			...
			if (valores.length != 2) {
				...
				med.lanzaraviso(mensaje, "Error");
			} else if (isNumeric(valores[0]) && isNumeric(valores[1])
					&& Integer.parseInt(valores[0]) < Integer.parseInt(valores[1]))
				return true;
	...
	private boolean checkIsNumeric() {
		...
		for (int i = 0; i < valores.length && ok; i++)
			if (valores[i][3] == null || String.valueOf(valores[i][3]).equalsIgnoreCase(""))
				ok = true;
			else if (String.valueOf(valores[i][5]).equalsIgnoreCase("Numerico"))
	...
	private boolean checkValorBinario() {
		...
		for (int i = 0; i < PreferenciasDefinicion.getNumCamposProtocolo() && ok; i++) {
			if (PreferenciasDefinicion.getValorTabla(i, 5).equals("Booleano")
					&& !PreferenciasDefinicion.getValorTabla(i, 4).equals("1"))
				ok = false;
